...
import com.fitpal.webapp.config.EmbeddedKafka;
import com.fitpal.webapp.config.KafkaSseConsumer;
import com.fitpal.webapp.config.KafkaSseProducer;
...
class FitpalKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/fitpal-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/fitpal-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/fitpal-kafka/unregister"));
                return;
            }
        }
